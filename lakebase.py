...
def run_write_returning(sql: str, params: tuple | dict | None = None) -> list[dict]:
    """Run a write that uses RETURNING, commit it, and return the rows.

    Needed because run_query() never commits (psycopg2 defaults to
    autocommit=False and get_connection() closes without committing), so an
    INSERT ... RETURNING sent through run_query would be silently rolled back.
    """
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, params)
