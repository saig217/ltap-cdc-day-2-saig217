...
if not username:
    raise ValueError("Set the username widget to your table suffix (same as the app login).")
...
print(f"  graph edges   : {graph_edges_table}")
print(f"  ai query cache: {ai_query_cache_table}")
print(f"  feedback (CDC): {feedback_history_table}")
...
def load_blocked_suggestions():
    """Read human feedback out of the CDC'd Delta table.

    Returns (blocked_set, blocked_by_source):
      blocked_set      -- {(source_repo, package_name, suggested_repo)} marked 'bad'
      blocked_by_source-- {source_repo: {suggested_repo, ...}} for negative prompting

    Why this is not the snippet from the assignment: that version selects every
    row where feedback = 'bad', which means a suggestion the student later
    changed to 'good' stays blocked forever, because the old 'bad' insert is
    still sitting in the change history. CDC is append-only -- you have to
    reduce to the latest change per key first, exactly like the repos query at
    the top of this notebook does.
    """
    try:
        blocked_df = spark.sql(f"""
            ...
            FROM latest
            WHERE rn = 1 AND feedback = 'bad'
        """)
        rows = blocked_df.collect()
    except Exception as e:
        print(f"  ⚠ Could not read feedback table ({e}); proceeding with no blocks")
        return set(), {}

    blocked_set = set()
    blocked_by_source = {}
    for r in rows:
        key = (r["source_repo"], r["package_name"], r["suggested_repo"])
        blocked_set.add(key)
        blocked_by_source.setdefault(r["source_repo"], set()).add(r["suggested_repo"])

    print(f"  ✓ Loaded {len(blocked_set)} blocked suggestions from human feedback")
    return blocked_set, blocked_by_source


def purge_blocked_from_cache(blocked_set):
    """Delete bad mappings from ai_query_cache so the AI re-queries next run.

    Uses MERGE rather than a per-row DELETE loop: one job instead of N, and it
    avoids relying on named-parameter binding inside a DELETE statement.
    """
    if not blocked_set:
        return 0
...
def discover_repos_from_dependencies_ai(staging_df, already_seen, client):
    ...
    Human-in-the-loop: suggestions marked 'bad' in the app are excluded from
    cache hits, purged from the cache, and explicitly forbidden in the re-query
    prompt.
    """
    global _github_request_count

    blocked_set, blocked_by_source = load_blocked_suggestions()
    purge_blocked_from_cache(blocked_set)
    ...
    for source_repo, dep_files in list(repo_deps.items())[:10]:
        # If a human rejected anything for this repo, skip the cache entirely.
        # The cache-hit branch is all-or-nothing per source_repo: deleting one bad
        # row still leaves the other mappings, so without this the AI would never
        # be re-queried and the "replacement suggestion" half of the loop is dead.
        rejected = blocked_by_source.get(source_repo, set())
        ...
        if cached_rows:
            ...
            skipped = len(cached_rows) - len(usable)
            print(f"    ✓ Cache hit for {source_repo} ({len(usable)} mappings, {skipped} blocked)")
            for row in usable:
                suggested = row["suggested_repo"]
                if "/" in suggested and suggested not in already_seen:
                    ai_suggestions.append((source_repo, row["package_name"], suggested))
        ...
        # No cache (or cache bypassed because of feedback) — call ai_query
        prompt = _AI_DEPENDENCY_PROMPT.format(
            source_repo=source_repo,
            dep_contents="\n\n".join(dep_files),
        )
        if rejected:
            # Bonus: negative prompting. Tell the model what a human already rejected
            # instead of letting it hand back the same repo it just got dinged for.
            prompt += (
                "\n\nIMPORTANT: A human reviewer rejected these suggestions as wrong, "
                "archived, or nonexistent. Do NOT suggest any of them again: "
                + ", ".join(sorted(rejected))
                + ". Suggest a different, correct repository for those packages instead."
            )
            print(f"    ↻ Re-querying {source_repo} with {len(rejected)} repo(s) excluded")
        try:
            result_df = spark.sql(
            ...
            if isinstance(suggested_map, dict):
                # Write package→repo mappings to cache, minus anything a human has
                # already rejected — otherwise the model repeats itself and we
                # immediately re-poison the cache we just purged.
                cache_rows = [
                ...
                for pkg, suggested in suggested_map.items():
                    if not (isinstance(suggested, str) and "/" in suggested):
                        continue
                    if (source_repo, pkg, suggested) in blocked_set:
                        continue
    ...
    for source, pkg, suggested_repo in unique_suggestions[:max_validations]:
        ...
        try:
            ...
            # package_name is the reason this edge exists AND the third column of the
            # ai_query_cache key. Without it in the metadata, the app cannot send a
            # feedback row that matches anything, and the whole loop no-ops.
