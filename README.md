...
- `POST /login` with `{"username": "ada"}` (or a form field) - validates the username as a safe
  Postgres table-name suffix, then starts a session. Tables live in the shared `bootcamp_students`
  schema as `<table>_<username>` (e.g. `bootcamp_students.github_repos_ada`)
...
## Student sign-in and shared Lakebase schema
...
Postgres identifiers like table names cannot be parameterized as query values, so the
username is validated *before* being interpolated into DDL).

All students share a single `bootcamp_students` schema. Tables are namespaced by appending the
username as a suffix (e.g. `github_repos_ada`, `github_files_ada`). This avoids per-student
schemas which would break CDF replication at scale.

On successful sign-in, the app validates the username and starts a session. Students create
the shared schema (idempotent) and their tables as a separate exercise by running the SQL files
in `sql/`:

```bash
sql/create_student_schema.sql       # CREATE SCHEMA IF NOT EXISTS bootcamp_students
sql/create_github_repos.sql         # CREATE TABLE bootcamp_students.github_repos_<username> + REPLICA IDENTITY FULL
sql/create_github_files.sql         # CREATE TABLE bootcamp_students.github_files_<username> + REPLICA IDENTITY FULL
```
